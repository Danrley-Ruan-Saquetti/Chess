...
export default function BoardControl() {
    ...
    const getDirection = ({ position, target }) => {
    ...
    const travelHouses = ({ position, target, direction, observer = () => { } }) => {
        for (let i = 1; target.x != position.x + i * direction.x || target.y != position.y + i * direction.y; i++) {
            ...
            if (responseObserver.valueOf) {
                return responseObserver.data;
            }
        }
        return null;
    };
