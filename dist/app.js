...
const App = () => {
    ...
    const selectPiece = ({ id }) => {
        ...
        if (!piece || !position || (piece.side != dataGame.data.sideTime)) {
            return;
        }
        pieceSelected = piece;
        isClicked = true;
    ...
    const activeSelectPromotion = () => {
        const houses = document.querySelectorAll(".type-promotions .house");
        houses.forEach(house => {
            house.addEventListener("click", selectPromotion);
        });
    };
    const selectPromotion = ({ target: tEl }) => {
    ...
    const validMovePiece = () => {
        if (!pieceSelected || !target) {
            return;
        }
        const { position, piece } = dataGame.getPiece(pieceSelected);
        if (!position || (position.x == target.x && position.y == target.y)) {
            return;
        }
