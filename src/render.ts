...
export default function ControlRender() {
    ...
    const getPieceImg = ({ type, side }: { type: TypePiece, side: TypeSide }) => {
    ...
    // DOM
    const appendEl = (parent: Element | null, children: Element) => {
        if (!parent) { return }

        parent.appendChild(children)
    }

    const createHouse = (isWhite: Boolean = false, board: Element | null, attr: { key: string, value: string }[]) => {
    ...
    const createPiece = (piece: IPiece) => {
    ...
    const movePiece = ({ id, target }: { target: BoardCoordinates, id: String }) => {
    ...
    const updatePiece = ({ piece }: { piece: IPiece }) => {
    ...
    const selectHouse = ({ position, className = "selected" }: { position: BoardCoordinates, className?: string }) => {
    ...
    const openTypePromotions = () => {
        ...
        const typePromotions: TypePiece[] = ["queen", "rook", "bishop", "horse"]

        const houses: { el: HTMLDivElement, type: TypePiece }[] = typePromotions.map(pi => {
            isWhite = !isWhite
    ...
    const togglePromotions = ({ side = "white", value }: { value: boolean, side?: TypeSide }) => {
        if (value) {
